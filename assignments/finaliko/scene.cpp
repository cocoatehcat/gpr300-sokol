...
Scene::Scene()
{
    ...
    //mist as a postprocess
    postProcessingEffects.push_back(std::make_unique<ew::Shader>("assets/shaders/fullscreen.vs", "assets/shaders/IkoFinalShaders/mistfade.fs"));
...
void Scene::Render(void)
{
    ...
    glBindFramebuffer(GL_FRAMEBUFFER, elevation_fbo);
    {
        ...
        elevation->setMat4("model", suzanne_matrix);
...
void Scene::Debug(void)
{
    ...
    if (ImGui::CollapsingHeader("Mist"))
    {
        ImVec2 uv_min(0.0f, 1.0f);
        ImVec2 uv_max(1.0f, 0.0f);

        ImGui::SliderFloat("Fog Height", &debug.fogHeight, -5.0f, 5.0f);
